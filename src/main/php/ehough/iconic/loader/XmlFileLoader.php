            ...
            switch ($arg->getAttribute('type')) {
                case 'service':
                    ...
                    if ($strict = $arg->getAttribute('strict')) {
                        $strict = self::_phpize($strict);
                    } else {
                        $strict = true;
                    }

                    $arguments[$key] = new ehough_iconic_Reference($arg->getAttribute('id'), $invalidBehavior, $strict);
                    break;
                case 'expression':
                    $arguments[$key] = new \Symfony\Component\ExpressionLanguage\Expression($arg->nodeValue);
                    break;
                case 'collection':
                ...
                case 'constant':
                    ...
                    break;
                default:
                    $arguments[$key] = self::_phpize($arg->nodeValue);
            }
        }
...
EOF
    ...
    private static function _phpize($o)
    {
        return call_user_func(array('\Symfony\Component\Config\Util\XmlUtils', 'phpize'), $o);
    }
}
